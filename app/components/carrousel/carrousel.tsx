...
export const Carrousel = ({ imagesArray = [] }: Props) => {
    ...
    return (
        <div className="w-full flex flex-col relative laptop:overflow-hidden h-[550px] mt-4">
            ...
            {imagesArray.map((image, index) =>
                <img
                    ...
                    className={`h-full max-h-[400px] object-contain w-full transform transition-all duration-500 ${index === selected ? "translate-y-0 opacity-100 " : "translate-y-full opacity-0 hidden"}`}
                />
            )}
